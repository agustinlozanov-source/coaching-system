      ...
      {toast && (
        <div
          className={`fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-full border px-5 py-2.5 text-sm font-semibold shadow-lg ${
            toast.type === 'success'
              ? 'border-emerald-500 bg-[#1c1c1e] text-emerald-400'
              : 'border-red-500 bg-[#1c1c1e] text-red-400'
          }`}
        >
          {toast.msg}
        </div>
...
function SetupForm({
  ...
  return (
    <div className="max-w-3xl">
      ...
      {/* META */}
      <div className="mb-4 rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-6">
        ...
        <div className="rounded-xl border-[1.5px] border-[#1aab99] bg-gradient-to-br from-[#1aab99]/10 to-[#3533cd]/10 p-5">
          ...
          <input
            ...
            className="w-full bg-transparent text-xl font-extrabold text-white outline-none placeholder:text-white/25"
          />
          <div className="mt-2.5 text-xs italic leading-relaxed text-white/40">
      ...
      <div className="mb-4 rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-6">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {[
            ...
            <div key={a.label} className="rounded-xl border border-white/10 bg-[#141416] p-4">
              ...
              <textarea
                ...
                placeholder={a.ph}
                className="min-h-[80px] w-full resize-none bg-transparent text-sm text-white outline-none placeholder:text-white/25"
              />
            </div>
          ))}
        </div>
      ...
      <div className="mb-6 rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/40">Fecha de inicio</label>
            <input type="date" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)} className={inputCls} />
            <div className="mt-1.5 text-xs text-white/40">El Round 1 empieza este día. Los 12 trimestres se calcularán automáticamente.</div>
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/40">Nombre del Vector (opcional)</label>
            <input value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej: Vector 2026-2028 · Triplicación" className={inputCls} />
            <div className="mt-1.5 text-xs text-white/40">Para identificarlo en tu historial.</div>
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="mb-6 rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-5">
        <div className="mb-4 flex items-center gap-2 text-xs font-extrabold uppercase tracking-wide text-white">
          <GitCommit className="h-3.5 w-3.5 text-[#1aab99]" /> Los 12 rounds — vista timeline
        </div>
        <div className="relative h-14 px-2">
          <div className="absolute left-2 right-2 top-[22px] h-1 rounded-full bg-white/10">
            <div
                  ...
                  className={`mt-3.5 mb-1.5 block rounded-full ${
                    t.estado === 'completado'
                      ...
                      : t.estado === 'activo'
                        ...
                        : 'h-3 w-3 border-2 border-white/20 bg-white/5'
                  }`}
                />
                <span className={`text-[10px] font-bold ${t.estado === 'activo' ? 'text-[#1aab99]' : t.estado === 'completado' ? 'text-emerald-400' : 'text-white/40'}`}>
                  R{t.numero}
                </span>
              </div>
            ))}
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-3">
        {anios.map((a) => {
          ...
          return (
            <div key={a.num} className={`relative rounded-2xl border bg-[#1c1c1e] p-5 ${borderCls}`}>
              <span className="absolute right-4 top-4 rounded-full bg-white/[0.06] px-2.5 py-1 text-[10px] font-extrabold text-white/40">{badge}</span>
              ...
              <div className="mb-2 text-base font-extrabold text-white">
                {a.plan ? planLines[0] : <span className="italic text-white/40">Sin plan definido</span>}
              </div>
              {a.plan && planLines.length > 1 && (
                <div className="mb-3 text-xs leading-relaxed text-white/50">{planLines.slice(1).join(' ')}</div>
              )}
              <div className="flex gap-3 border-t border-white/10 pt-3 text-xs text-white/40">
                <span><strong className="text-white/60">{comp}/4</strong> rounds</span>
                {activoDelAnio ? (
                  <span><strong className="text-white/60">R{activoDelAnio.numero}</strong> activo</span>
                ) : a.rounds.length === 4 ? (
        ...
        <>
          ...
          >
            <div className="min-w-0">
              <div className="mb-1.5 flex items-center gap-2.5 text-xs text-white/50">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {trimestres.map((t) => {
          ...
          const borderCls = t.estado === 'activo' ? 'border-[#1aab99] shadow-[0_0_0_1px_#1aab99]' : t.estado === 'completado' ? 'border-emerald-500/50' : 'border-white/10';
          const bgCls = t.estado === 'activo' ? 'bg-gradient-to-br from-[#1aab99]/10 to-transparent' : t.estado === 'completado' ? 'bg-gradient-to-br from-emerald-500/[0.06] to-transparent' : 'bg-[#1c1c1e]';
          return (
            <button
              key={t.id}
              onClick={() => onOpenTrimestre(t.id)}
              className={`flex min-h-[130px] flex-col rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 hover:border-white/30 ${borderCls} ${bgCls}`}
            >
              <div className="mb-2.5 flex items-center justify-between">
                ...
                <span className="text-[10.5px] font-semibold text-white/40">Q{t.trimestre_anio}</span>
              </div>
              <div className={`mb-1.5 text-sm font-bold leading-snug ${t.estado === 'completado' ? 'text-white/50' : 'text-white'}`}>
                {t.titulo || 'Sin definir'}
              </div>
              <div className="mb-auto text-[11px] italic leading-snug text-white/40">
                {fx?.complemento ? `Factor X: Utilidad por ${fx.complemento}` : 'Factor X sin definir'}
              </div>
              <div className="mt-2.5 flex justify-between border-t border-white/10 pt-2.5">
                {t.estado === 'activo' && (
                ...
                {t.estado === 'pendiente' && (
                  <span className="rounded-full bg-white/[0.06] px-2.5 py-1 text-[10.5px] font-bold text-white/40">Pendiente</span>
                )}
              </div>
            </button>
          );
