...
export function IndicadorModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-white/10 bg-[#161618]">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-[#161618] px-6 py-4">
          <h2 className="text-base font-extrabold text-white">{editing ? 'Editar indicador' : 'Nuevo indicador crítico'}</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4 px-6 py-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/40">Responsable</label>
                <input value={responsable} onChange={(e) => setResponsable(e.target.value)} placeholder="Nombre de la persona" className={inputCls} />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/40">Frecuencia</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/40">Dirección</label>
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/40">Unidad (opcional)</label>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {[
                  ...
                  <div key={u.label} className={`rounded-lg border p-2.5 ${u.border}`}>
                    <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-extrabold text-white">
                      <span className={`h-2.5 w-2.5 rounded-full ${u.dot}`} /> {u.label}
                    </div>
                    <input
                      type="number" step="any" value={u.value} onChange={(e) => u.set(e.target.value)}
                      placeholder="0"
                      className="w-full rounded-md border border-white/10 bg-[#0d0d10] px-2.5 py-1.5 text-sm font-bold text-white outline-none focus:border-[#1aab99]"
                    />
                    <div className="mt-1 text-[10.5px] font-semibold text-white/40">{u.hint}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="sticky bottom-0 flex justify-end gap-2.5 border-t border-white/10 bg-[#161618] px-6 py-4">
