    ...
    <div>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="text-2xl font-bold text-white">Resumen y Punto de Equilibrio</h1>
        </div>
      </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-br from-[#1aab99]/10 to-[#3533cd]/10 p-4">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-white/40">Margen promedio</div>
          <div className="mt-1 text-2xl font-extrabold text-white">{fmtPct(kpis.promedio)}</div>
        </div>
        <div className="rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-4">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-white/40">Rentables ≥30%</div>
          <div className="mt-1 text-2xl font-extrabold text-emerald-400">{kpis.rentables}</div>
          <div className="text-xs text-white/40">{kpis.total > 0 ? Math.round((kpis.rentables / kpis.total) * 100) : 0}% del catálogo</div>
        </div>
        <div className="rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-4">
          ...
          <div className="text-xs text-white/40">Margen &lt; 30%</div>
        </div>
        <div className="rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-4">
          ...
          <div className="text-xs text-white/40">Precio &lt; costo</div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-5">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-white">Ranking de rentabilidad</h2>
            </div>
            {ranking.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {ranking.map((d, idx) => {
                  ...
                  const medal = pos === 1 ? 'text-amber-400' : pos === 2 ? 'text-white/50' : pos === 3 ? 'text-orange-400' : 'text-white/30';
                  return (
                    <div key={d.id} className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-[#141416] p-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {d.categoria && <div className="text-xs text-white/40">{d.categoria}</div>}
                      </div>
                      <div className="hidden gap-4 text-right text-xs sm:flex">
                        <div><div className="text-white/40">Precio</div><div className="font-semibold text-white">{fmtMoney(d.precio)}</div></div>
                        <div><div className="text-white/40">Costo</div><div className="font-semibold text-white">{fmtMoney(d.costoTotal)}</div></div>
                        <div><div className="text-white/40">Utilidad</div><div className={`font-semibold ${d.utilidad >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{fmtMoney(d.utilidad)}</div></div>
                      </div>
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-5">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-white">Punto de equilibrio</h2>
            </div>
            {!puntoEquilibrio ? (
              <div className="flex items-center gap-2.5 rounded-xl border border-white/[0.06] bg-[#141416] p-4 text-sm text-white/50">
              ...
              <>
                <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <div className="rounded-xl border border-white/10 bg-[#141416] p-3">
                    <div className="text-[10px] uppercase text-white/40">Ventas mínimas</div>
                    <div className="mt-1 font-extrabold text-white">{fmtMoney(puntoEquilibrio.ventasMinimas)}</div>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-[#141416] p-3">
                    <div className="text-[10px] uppercase text-white/40">Gastos fijos</div>
                    <div className="mt-1 font-extrabold text-white">{fmtMoney(puntoEquilibrio.totalGastos)}</div>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-[#141416] p-3">
                    <div className="text-[10px] uppercase text-white/40">Margen contrib.</div>
                    <div className="mt-1 font-extrabold text-white">{fmtPct(puntoEquilibrio.mcPromedio)}</div>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-[#141416] p-3">
                    <div className="text-[10px] uppercase text-white/40">Unidades / mes</div>
                    <div className="mt-1 font-extrabold text-white">{fmtNum(puntoEquilibrio.unidadesMinimas, 0)}</div>
                  </div>
                </div>
                        ...
                        {unidadesPorProducto.map((d) => {
                          ...
                          return (
                            <tr key={d.id} className="border-b border-white/[0.04] last:border-0">
                              <td className="px-3 py-2 font-semibold text-white">{d.nombre}</td>
                              <td className="px-3 py-2 text-right text-white/70">{fmtMoney(d.contribucionUnit)}</td>
                              <td className="px-3 py-2 text-right text-white/70">{fmtNum(unidadesMes, 0)}</td>
                              <td className="px-3 py-2 text-right text-white/70">{fmtNum(unidadesDia, 0)}</td>
                            </tr>
                          );
                        })}
                      </tbody>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-2xl border border-white/[0.08] bg-[#1c1c1e] p-5">
            <div className="mb-1 flex items-center justify-between">
              <h2 className="text-sm font-bold text-white">Simulador &quot;¿qué pasaría si...?&quot;</h2>
              <button onClick={resetSim} title="Reiniciar" className="flex h-7 w-7 items-center justify-center rounded-lg text-white/40 transition hover:bg-white/[0.06] hover:text-white">
                <RotateCcw className="h-3.5 w-3.5" />
              </button>
            </div>
            <p className="mb-4 text-xs leading-relaxed text-white/50">Simulación en vivo, no se guarda. Ajusta y mira el impacto en el ranking y el punto de equilibrio.</p>
  ...
  return (
    <div className="mb-4 last:mb-0">
      <div className="mb-1.5 flex items-center justify-between text-xs">
        ...
        <span className={`font-bold ${value > 0 ? 'text-emerald-400' : value < 0 ? 'text-red-400' : 'text-white/60'}`}>{value >= 0 ? '+' : ''}{value}%</span>
      </div>
      <input
