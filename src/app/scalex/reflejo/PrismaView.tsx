  ...
  return (
    <>
      ...
      <div className="mb-6 flex items-center gap-5 rounded-2xl border border-amber-500/40 bg-gradient-to-br from-amber-500/10 to-orange-600/5 p-7">
        ...
        <div>
          <h2 className="mb-1.5 text-lg font-extrabold text-[var(--sx-text)]">Aún te faltan herramientas para generar PRISMA</h2>
          <p className="text-sm leading-relaxed text-[var(--sx-text-muted)]">PRISMA es la integración de los pilares anteriores. Necesitas completar primero el PIE y el MAPE para que el sistema pueda cruzar los datos.</p>
        </div>
      </div>
...
function RequisitoPill({
  ...
  return (
    <div className={`flex flex-col items-center gap-4 rounded-2xl border p-6 text-center sm:flex-row sm:text-left ${done ? 'border-emerald-500/40 bg-gradient-to-br from-emerald-500/[0.06] to-transparent' : 'border-[var(--sx-border)] bg-[var(--sx-card)]'}`}>
      ...
      <div className="flex-1">
        <div className="mb-1 text-base font-extrabold text-[var(--sx-text)]">{titulo}</div>
        <div className="mb-2 text-xs leading-relaxed text-[var(--sx-text-dim)]">{desc}</div>
        <div className={`flex items-center justify-center gap-1.5 text-xs font-semibold sm:justify-start ${done ? 'text-emerald-400' : 'text-[var(--sx-text-dim)]'}`}>
...
function Listo({
  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-3.5 sm:grid-cols-2">
        <div className="rounded-2xl border border-[var(--sx-border)] bg-[var(--sx-card)] p-5">
          <div className="mb-3.5 flex items-center gap-3">
            ...
            <div className="text-[11px] font-extrabold uppercase tracking-wide text-[var(--sx-text-dim)]">PIE — Tu liderazgo</div>
          </div>
          <div className="mb-3 text-lg font-extrabold text-[var(--sx-text)]">{requisitos.pie_perfil ? PIE_PERFIL_NOMBRES[requisitos.pie_perfil] ?? requisitos.pie_perfil : '-'}</div>
          <div className="flex justify-between border-t border-[var(--sx-border)] pt-3 text-xs text-[var(--sx-text-dim)]">
            <span>Completado</span><strong className="text-[var(--sx-text-muted)]">{fmtFecha(requisitos.pie_fecha)}</strong>
          </div>
        </div>
        <div className="rounded-2xl border border-[var(--sx-border)] bg-[var(--sx-card)] p-5">
          <div className="mb-3.5 flex items-center gap-3">
            ...
            <div className="text-[11px] font-extrabold uppercase tracking-wide text-[var(--sx-text-dim)]">MAPE — Tu empresa</div>
          </div>
          <div className="mb-3 text-lg font-extrabold text-[var(--sx-text)]">{requisitos.mape_cuadrante ? MAPE_CUADRANTE_NOMBRES[requisitos.mape_cuadrante] ?? requisitos.mape_cuadrante : '-'}</div>
          <div className="flex justify-between border-t border-[var(--sx-border)] pt-3 text-xs text-[var(--sx-text-dim)]">
            <span>Completado</span><strong className="text-[var(--sx-text-muted)]">{fmtFecha(requisitos.mape_fecha)}</strong>
          </div>
        </div>
      </div>
      ...
      {historial.length > 0 && (
        <div className="rounded-2xl border border-[var(--sx-border)] bg-[var(--sx-card)] p-5">
          <div className="mb-3.5 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-[var(--sx-text)]">
          ...
          <div className="flex flex-col gap-2">
            {historial.map((s) => {
              ...
              return (
                <div key={s.id} className="flex items-center gap-3.5 rounded-xl border border-[var(--sx-border)] bg-[var(--sx-input)] p-3">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-bold text-[var(--sx-text)]">{perfil?.nombre ?? s.perfil_prisma}</div>
                    <div className="text-xs text-[var(--sx-text-dim)]">{fmtFecha(s.generado_en)}</div>
                  </div>
                  <button onClick={() => onVer(s.id)} className="flex items-center gap-1.5 rounded-full border border-[var(--sx-border)] bg-[var(--sx-card-hover)] px-3 py-1.5 text-xs font-bold text-[var(--sx-text)] transition hover:bg-[var(--sx-border)]">
...
function Resultado({
  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-3.5 sm:grid-cols-2">
        <div className="flex items-center gap-3.5 rounded-2xl border border-[var(--sx-border)] bg-[var(--sx-card)] p-4">
          ...
          <div>
            <div className="text-[10.5px] font-extrabold uppercase tracking-wide text-[var(--sx-text-dim)]">PIE — Tu liderazgo</div>
            <div className="text-sm font-extrabold text-[var(--sx-text)]">{PIE_PERFIL_NOMBRES[snapshot.pie_perfil] ?? snapshot.pie_perfil}</div>
            <div className="text-xs text-[var(--sx-text-dim)]">Puntaje: <strong className="text-[var(--sx-text-muted)]">{snapshot.pie_puntaje_total} / 100</strong></div>
          </div>
        </div>
        <div className="flex items-center gap-3.5 rounded-2xl border border-[var(--sx-border)] bg-[var(--sx-card)] p-4">
          ...
          <div>
            <div className="text-[10.5px] font-extrabold uppercase tracking-wide text-[var(--sx-text-dim)]">MAPE — Tu empresa</div>
            <div className="text-sm font-extrabold text-[var(--sx-text)]">{MAPE_CUADRANTE_NOMBRES[snapshot.mape_cuadrante] ?? snapshot.mape_cuadrante}</div>
            <div className="text-xs text-[var(--sx-text-dim)]">F: <strong className="text-[var(--sx-text-muted)]">{snapshot.mape_puntaje_financiero}</strong> / O: <strong className="text-[var(--sx-text-muted)]">{snapshot.mape_puntaje_operativo}</strong></div>
          </div>
        </div>
      </div>

      <div className="mb-6">
        <h2 className="mb-3.5 flex items-center gap-2 text-lg font-extrabold text-[var(--sx-text)]">
      ...
      <div className="mb-6 rounded-2xl border border-[var(--sx-border)] bg-[var(--sx-card)] p-5">
        <div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-[var(--sx-text)]">
        ...
        <textarea
          ...
          className="min-h-[90px] w-full resize-y rounded-lg border border-[var(--sx-border)] bg-[var(--sx-input)] px-3.5 py-3 text-sm text-[var(--sx-text)] outline-none transition placeholder:text-[var(--sx-text-faint)] focus:border-[#1aab99] focus:ring-2 focus:ring-[#1aab99]/25"
        />
        <div className="mt-1.5 text-[11px] text-[var(--sx-text-faint)]">Las notas se guardan automáticamente cada 1.2 segundos.</div>
      </div>

      <div className="flex flex-wrap gap-2.5">
        <button onClick={onGenerarNuevo} className="flex items-center gap-2 rounded-full border border-[var(--sx-border)] bg-[var(--sx-card-hover)] px-5 py-2.5 text-sm font-bold text-[var(--sx-text)] transition hover:bg-[var(--sx-border)]">
...
function HorizonteCard({
  ...
  return (
    <div className={`overflow-hidden rounded-2xl border bg-[var(--sx-card)] transition-colors ${expanded ? 'border-[var(--sx-border-strong)]' : 'border-[var(--sx-border)]'}`}>
      <div onClick={onToggle} className="grid cursor-pointer grid-cols-[64px_1fr_auto_auto] items-center gap-4 p-5 transition hover:bg-[var(--sx-card-hover)] sm:grid-cols-[90px_1fr_auto_auto]">
        <div>
          ...
          <div className="mt-1 text-[11px] font-bold uppercase tracking-wide text-[var(--sx-text-dim)]">días</div>
        </div>
        <div className="min-w-0">
          <div className="mb-1.5 text-sm font-extrabold text-[var(--sx-text)]">{titulo}</div>
          <div className="line-clamp-2 text-sm text-[var(--sx-text-muted)]">{texto || '-'}</div>
        </div>
        {stat && stat.total > 0 && (
          <div className="hidden items-center gap-2 rounded-full bg-[var(--sx-card-hover)] px-3 py-1.5 sm:flex">
            <span className="whitespace-nowrap text-xs font-extrabold text-[var(--sx-text)]">{stat.hechas}/{stat.total}</span>
            <div className="h-1.5 w-[50px] overflow-hidden rounded-full bg-[var(--sx-border-strong)]">
              <div className="h-full rounded-full bg-emerald-500 transition-all" style={{ width: `${stat.pct}%` }} />
            </div>
          </div>
        )}
        <button type="button" className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg border border-[var(--sx-border)] bg-[var(--sx-card-hover)] text-[var(--sx-text-dim)] transition-transform ${expanded ? 'rotate-180 text-[#1aab99]' : ''}`}>
          <ChevronDown className="h-4 w-4" />
        </button>
      </div>

      {expanded && (
        <div className="border-t border-[var(--sx-border)] px-5 pb-5 pt-4">
          {acciones.length === 0 ? (
            <div className="mb-3.5 rounded-lg border border-dashed border-[var(--sx-border-strong)] bg-[var(--sx-input)] p-4 text-center text-xs leading-relaxed text-[var(--sx-text-dim)]">
            ...
            <div className="mb-3.5 flex flex-col gap-2">
              {acciones.map((a) => {
                ...
                return (
                  <div
                    key={a.id}
                    className={`grid grid-cols-[36px_1fr_auto] items-start gap-3.5 rounded-lg border bg-[var(--sx-input)] p-3.5 transition ${vencida ? 'border-red-500/50 bg-red-500/[0.04]' : 'border-[var(--sx-border)]'} ${a.estado === 'hecha' ? 'opacity-60' : ''} ${a.estado === 'descartada' ? 'opacity-40' : ''}`}
                  >
                    <button
                      ...
                      className={`mt-0.5 flex h-8 w-8 items-center justify-center rounded-full border transition ${
                        a.estado === 'hecha' ? 'border-emerald-500 bg-emerald-500/15 text-emerald-400'
                          ...
                          : 'border-[var(--sx-border-strong)] bg-[var(--sx-card-hover)] text-[var(--sx-text-dim)] hover:bg-[var(--sx-border)]'
                      }`}
                    >
                      <EstadoIcon className="h-4 w-4" />
                    </button>
                    <div className="min-w-0">
                      <div className={`text-sm font-bold text-[var(--sx-text)] ${a.estado === 'hecha' ? 'text-[var(--sx-text-muted)] line-through' : ''}`}>{a.titulo}</div>
                      {a.descripcion && <div className="mt-1 text-xs leading-relaxed text-[var(--sx-text-dim)]">{a.descripcion}</div>}
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {a.fecha_limite && (
                          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-semibold ${vencida ? 'border-red-500/40 bg-red-500/15 text-red-400' : 'border-[var(--sx-border)] bg-[var(--sx-card-hover)] text-[var(--sx-text-muted)]'}`}>
                        ...
                        <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-semibold ${
                          a.estado === 'hecha' ? 'border-emerald-500/40 bg-emerald-500/15 text-emerald-400'
                            ...
                            : 'border-[var(--sx-border)] bg-[var(--sx-card-hover)] text-[var(--sx-text-muted)]'
                        }`}>
                          <EstadoIcon className="h-2.5 w-2.5" /> {ESTADO_LABELS[a.estado]}
                        </span>
                        {a.impacto_esperado && (
                          <span className="inline-flex items-center gap-1 rounded-full border border-[var(--sx-border)] bg-[var(--sx-card-hover)] px-2 py-0.5 text-[11px] font-semibold text-[var(--sx-text-muted)]">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => onEditar(a)} title="Editar" className="flex h-7 w-7 items-center justify-center rounded-md border border-[var(--sx-border)] bg-[var(--sx-card-hover)] text-[var(--sx-text-dim)] transition hover:bg-[var(--sx-border)] hover:text-[var(--sx-text)]">
                        <Edit2 className="h-3 w-3" />
                      </button>
                      <button
                        onClick={() => { if (confirm(`¿Eliminar "${a.titulo}"? Esta acción no se puede deshacer.`)) onEliminar(a.id); }}
                        title="Eliminar"
                        className="flex h-7 w-7 items-center justify-center rounded-md border border-[var(--sx-border)] bg-[var(--sx-card-hover)] text-[var(--sx-text-dim)] transition hover:border-red-500/40 hover:bg-red-500/15 hover:text-red-400"
                      >
          ...
          <button
            onClick={onCrear}
            className="flex w-full items-center justify-center gap-2 rounded-lg border-[1.5px] border-dashed border-[var(--sx-border-strong)] py-3 text-xs font-bold text-[var(--sx-text-dim)] transition hover:border-[#1aab99] hover:bg-[#1aab99]/[0.06] hover:text-[#1aab99]"
          >
...
function AccionModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-[var(--sx-border)] bg-[var(--sx-input)] shadow-2xl">
        <div className="sticky top-0 z-[1] flex items-center justify-between gap-3 border-b border-[var(--sx-border)] bg-[var(--sx-input)] px-6 py-5">
          <div className="flex items-center gap-3">
            <h2 className="text-[17px] font-extrabold text-[var(--sx-text)]">{accion ? 'Editar acción' : 'Nueva acción'}</h2>
            <span className={`rounded-full px-2.5 py-1 text-[11px] font-extrabold ${badgeCls}`}>{horizonte} DÍAS</span>
          </div>
          <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--sx-border)] bg-[var(--sx-card-hover)] text-[var(--sx-text-dim)] transition hover:bg-[var(--sx-border)] hover:text-[var(--sx-text)]">
        ...
        <form onSubmit={handleSubmit}>
          <div className="flex flex-col gap-3.5 px-6 py-5">
            <div>
              <label className="mb-1.5 block text-[11.5px] font-bold uppercase tracking-wide text-[var(--sx-text-muted)]">
              ...
              <input
                ...
                className="w-full rounded-lg border border-[var(--sx-border)] bg-[var(--sx-page)] px-3 py-2.5 text-sm text-[var(--sx-text)] outline-none transition placeholder:text-[var(--sx-text-faint)] focus:border-[#1aab99] focus:ring-2 focus:ring-[#1aab99]/25"
              />
              {error && <div className="mt-1 text-xs text-red-400">{error}</div>}
            </div>

            <div>
              <label className="mb-1.5 block text-[11.5px] font-bold uppercase tracking-wide text-[var(--sx-text-muted)]">Descripción</label>
              <textarea
                value={descripcion} onChange={(e) => setDescripcion(e.target.value)}
                placeholder="Detalle de qué hacer, contexto, condiciones específicas..."
                className="min-h-[60px] w-full resize-y rounded-lg border border-[var(--sx-border)] bg-[var(--sx-page)] px-3 py-2.5 text-sm text-[var(--sx-text)] outline-none transition placeholder:text-[var(--sx-text-faint)] focus:border-[#1aab99] focus:ring-2 focus:ring-[#1aab99]/25"
              />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1.5 block text-[11.5px] font-bold uppercase tracking-wide text-[var(--sx-text-muted)]">Fecha límite</label>
                <input
                  type="date" value={fechaLimite ?? ''} onChange={(e) => setFechaLimite(e.target.value)}
                  className="w-full rounded-lg border border-[var(--sx-border)] bg-[var(--sx-page)] px-3 py-2.5 text-sm text-[var(--sx-text)] outline-none transition focus:border-[#1aab99] focus:ring-2 focus:ring-[#1aab99]/25"
                />
              </div>
              <div>
                <label className="mb-1.5 block text-[11.5px] font-bold uppercase tracking-wide text-[var(--sx-text-muted)]">Estado</label>
                <select
            ...
            <div>
              <label className="mb-1.5 block text-[11.5px] font-bold uppercase tracking-wide text-[var(--sx-text-muted)]">Impacto esperado</label>
              <textarea
            ...
            <div>
              <label className="mb-1.5 block text-[11.5px] font-bold uppercase tracking-wide text-[var(--sx-text-muted)]">Evidencia / comprobante</label>
              <input
                value={evidencia ?? ''} onChange={(e) => setEvidencia(e.target.value)}
                placeholder="URL, descripción de un documento, foto de la reunión..."
                className="w-full rounded-lg border border-[var(--sx-border)] bg-[var(--sx-page)] px-3 py-2.5 text-sm text-[var(--sx-text)] outline-none transition placeholder:text-[var(--sx-text-faint)] focus:border-[#1aab99] focus:ring-2 focus:ring-[#1aab99]/25"
              />
              <div className="mt-1 text-[11px] text-[var(--sx-text-faint)]">Dónde quedó registro de que se hizo (link, ubicación del documento, etc).</div>
            </div>
          </div>

          <div className="sticky bottom-0 flex justify-end gap-2.5 border-t border-[var(--sx-border)] bg-[var(--sx-input)] px-6 py-4">
            <button type="button" onClick={onClose} className="rounded-full border border-[var(--sx-border)] bg-[var(--sx-card-hover)] px-5 py-2.5 text-sm font-bold text-[var(--sx-text)] transition hover:bg-[var(--sx-border)]">
              Cancelar
            </button>
            <button
