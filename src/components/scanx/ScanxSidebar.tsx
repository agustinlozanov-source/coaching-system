...
import {
  LayoutDashboard, ClipboardList, Radar, Users2, BarChart3, LogOut, Sun, Moon,
} from 'lucide-react';
...
const menuItems = [
  { label: 'Inicio', icon: LayoutDashboard, href: '/scanx' },
  { label: 'Diagnósticos', icon: ClipboardList, href: '/scanx/diagnosticos' },
  { label: 'Resultados', icon: Radar, href: '/scanx/resultados' },
];

const proximamente = [
  { label: 'Multiperspectiva', icon: Users2 },
  { label: 'Benchmarks', icon: BarChart3 },
];

export function ScanxSidebar({ theme, onToggleTheme }: { theme?: 'light' | 'dark'; onToggleTheme?: () => void }) {
    ...
    <div className="flex h-full flex-col border-r bg-card">
      ...
      <div className="border-b p-6">
        <Link href="/launcher" className="block">
          ...
          <img
            src={theme === 'dark' ? '/logos/scanx-blanco.png' : '/logos/scanx-negro.png'}
            alt="SCANx"
      ...
      <nav className="flex-1 space-y-1 p-4">
        {menuItems.map((item) => {
          ...
          const isActive = item.href === '/scanx'
            ? pathname === '/scanx'
        ...
        <div className="px-3 pb-1 pt-4 text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60">
          Próximamente
        </div>
